...
| `Edges.tsx` | 모든 엣지를 튜브 메시 하나로 (`tubes.ts`: 엣지당 `from→mid→to` 2차 베지어를 링으로 샘플링, 반지름 0.04). 정점 색으로 종류 표시, 선택과 닿은 엣지만 밝게. `MeshBasicMaterial`에 `onBeforeCompile`로 음영·흐름 대시(`aT`·`aDir`, `uTime`·`uFlow`)와 **추론 엣지 점선**(`aDash`·`aDerived`, 곡선 길이 기준 `discard`)을 얹는다. `flow`가 켜지면 `useFrame`에서 시간만 갱신 | O |
...
| `Edges.tsx` | Every edge in one tube mesh (`tubes.ts`: quadratic Bézier `from→mid→to` sampled into rings, radius 0.04). Vertex colours carry the kind; only edges touching the selection are lit. A `MeshBasicMaterial` patched via `onBeforeCompile` adds shading, the flow dashes (`aT` / `aDir`, `uTime` / `uFlow`) and the **dashed pattern for inferred edges** (`aDash` / `aDerived`, `discard` by arc length). With `flow` on, `useFrame` only advances the time | yes |
