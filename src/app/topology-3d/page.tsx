...
// 3D topology page. Three columns: source panel (collapsible), the scene, and
// the inspector. The page owns filter, expanded-subnet and selection state and
// feeds the scene a Layout3D computed from applyFilter → computeLayout, both
// memoised. The scene is client-only (three.js), so it is loaded with ssr:false.
// 3D 토폴로지 페이지. 3열 레이아웃. 필터·펼침·선택 상태는 여기서 들고, 씬에는 Layout3D만 준다.
import { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import dynamic from 'next/dynamic';
import { ChevronsDownUp, Gauge, Maximize2, PanelLeftClose, PanelLeftOpen } from 'lucide-react';
...
import { applyFilter, createDefaultFilter, resolveVpcId, type TopologyFilter } from '@/lib/topology/filter';
...
export default function Topology3dPage() {
  ...
  const [filter, setFilter] = useState<TopologyFilter>(() => createDefaultFilter());
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 px-4 py-2 border-b border-navy-600 bg-navy-800/60 text-xs">
        ...
        <label className="flex items-center gap-1.5 text-gray-400">
          ...
          <select
            ...
            onChange={(e) => setFilter((f) => ({ ...f, vpcId: e.target.value || null }))}
      ...
      <div className="flex flex-1 min-h-0">
        {leftOpen && (
          <aside className="w-80 shrink-0 overflow-y-auto border-r border-navy-600 p-3">
        ...
        <aside className="w-80 shrink-0 overflow-y-auto border-l border-navy-600 p-3">
          <Inspector
            graph={filtered}
            layout={layout}
            selectedId={selectedId}
