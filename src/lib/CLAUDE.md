...
- `topology/` — 3D/FossFLOW 공용 `TopologyGraph` 계약, `applyFilter`, `validateGraph`, 어댑터 3종(live/fixture/generator), `anonymize.ts`, `layout3d.ts`(3D 좌표, ADR-012), `chat.ts`·`sse.ts`(필터 채팅 순수 부분, ADR-013) (ADR-010, vitest)
...
- `topology/` — `TopologyGraph` contract shared by 3D and FossFLOW views, `applyFilter`, `validateGraph`, three adapters (live/fixture/generator), `anonymize.ts`, `layout3d.ts` (3D coordinates, ADR-012), `chat.ts` / `sse.ts` (pure side of the filter chat, ADR-013) (ADR-010, vitest)
